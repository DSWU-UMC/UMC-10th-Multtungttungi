...
function useGetLpList({ cursor, search, order, limit }: PaginationDto) {
  return useQuery({
    ...
    // 5 분 동안 기존 데이터를 그대로 활용해서 네트워크 요청을 줄인다.
    staleTime: 1000 * 60 * 5, // 5분

    // 사용되지 않는 비활성상태인 쿼리 데이터가 캐시에 남아이쓴ㄴ 시간.
    ...
    // retry: 쿼리 요청이 실패했을 때 자동으로 재시도할 횟수를 저장합니다.
    // 기본값은 3회 정도, 네트워크 오류 등 임시적인 문제를 보완할 수 있습니다.

    // intialData: 쿼리 실행 전 미리 제공할 초기 데이터를 설정합니다.
    ...
    // ex) 페이지네이션 시 페이지 전환 사이에 이전 데이터를 보여주며 사용자 경험을 향상시킨다.
