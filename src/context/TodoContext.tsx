...
export const TodoProvider = ({children}: {children}: 
    PropsWithChildren) : Element => {
        ...
        const deleteTodo = (todos:TTodo) : void => {
            setDoneTodos((prevDoneTodo) : TTodo[] =>
            prevDoneTodo.filter((t) : boolean => t.id !== todos.id)
        );
        };
        return 
        (<TodoContext.Provider value={{todos, doneTodos, 
        addTodo, completeTodo, deleteTodo }}>{children}  
        ...
        //컨텍스트가 없는 경우
        if (!context) {
            throw new Error('useTodo를 사용하기 위해서는, 무조건 TodoProvier로 감싸야 합니다.'
